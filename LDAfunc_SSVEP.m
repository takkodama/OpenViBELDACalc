function [z, d, p_4duration, b_coef] = LDAfunc_SSVEP(TrainTarget, TrainNonTarget, TrialDATA) 
...
%Discrinimant score
z = [ones(length(x),1) x] * b_coef';

%Distance from each hyperplane
for j = 1:(size(b_coef, 1))
    for i = 1:size(x, 1)
